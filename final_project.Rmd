...
## Remove NA values
```{r}
us_data = regressors_join[regressors_join$Country.Name == "United States",]
cleaned_us_data = us_data[complete.cases(us_data),]
```

## Outlier Detection

```{r}
us_target = cleaned_us_data[cleaned_us_data$Category == "SHARE.RE.IN.ELECTRICITY",]
x = unlist(us_target, use.names = FALSE)
X = as.numeric(x[3:28])
...
outliers = t(which(X < lower_bound_q | X > upper_bound_q))
sum = length(outliers)
us_ts = ts(X, start = 1990, end = 2015)
plot(us_ts)
print(sum)
...
us2014_ts = ts(y_10[c(1:24),c(3)], start = 1990, end = 2014)
